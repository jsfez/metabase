...
(defn- entity-context
  ...
  fields: `description`, `collection_id`, and the owner (transform + personal collection). `document`
  has no description; `collection` derives its breadcrumb anchor from `location` (see
  [[collection-context]])."
  [entity-type ids]
